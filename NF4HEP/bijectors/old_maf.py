...
from tensorflow_probability.python.bijectors import RationalQuadraticSpline  # type: ignore
from tensorflow_probability.python.internal import prefer_static as ps  # type: ignore
from tensorflow_probability.python.internal import tensorshape_util  # type: ignore

tfd = tfp.distributions
tfb = tfp.bijectors

from NF4HEP import Verbosity, print
...
class MAFNetwork_default(tfb.AutoregressiveNetwork, Verbosity): # type: ignore
    """
    ...
    def __init__(self,
        ...
        print(footer_string, "\nMAF default neural network architecture (tfb.AutoregressiveNetwork) set in ", str(end-start), "s.\n", show = verbose)
...
class MAFBijector_custom(tfb.Bijector, Verbosity):
    ...
    def get_config(self):
        ...
        config.update({
            "model_define_inputs": self._model_define_inputs,
            "model_bijector_inputs": self._model_bijector_inputs,
            "verbose": self.verbose,
        })
        return config
    ...
    def __set_inputs(self, verbose=None):
        ...
        try:
            ...
            raise Exception("model_define_inputs dictionary should contain at least a key 'params'.")
        utils.check_set_dict_keys(self._model_define_inputs, ["default_NN"], [True], verbose=verbose_sub)
        utils.check_set_dict_keys(self.model_bijector_inputs, ["is_constant_jacobian",
                                                           "validate_args",
                                                           "unroll_loop",
                                                           "event_ndims",
                                                           "name",
                                                          ...
                                                          [False,
                                                           ...
                                                           2,
                                                           -1])
        self._is_constant_jacobian = self.model_bijector_inputs["is_constant_jacobian"]
        self._validate_args = self.model_bijector_inputs["validate_args"]
        self._unroll_loop = self.model_bijector_inputs["unroll_loop"]
        self._event_ndims = self.model_bijector_inputs["event_ndims"]
        self._name = self.model_bijector_inputs["name"]
        self._spline_knots = self.model_bijector_inputs["spline_knots"]
        self._range_min = self.model_bijector_inputs["range_min"]
...
class MAFFlow(tfb.Chain, Verbosity):
    ...
    def __init__(self,
        ...
        default_bijector = self.model_bijector_inputs["default_bijector"]
        batch_norm = self.model_define_inputs.pop("batch_norm")
        ndims = self.model_bijector_inputs["ndims"]
        if default_bijector:
            bijector = MAFBijector_default(self.model_define_inputs)
        else:
            self.model_define_inputs["params"] = 3*self.model_bijector_inputs["spline_knots"]-1
            bijector = MAFBijector_custom(self.model_define_inputs, self.model_bijector_inputs)
        ...
        bijectors = []
        for _ in range(self.model_bijector_inputs["num_bijectors"]):
            if batch_norm:
    ...
    def __set_inputs(self, verbose=None):
        ...
        try:
            ...
            raise Exception("model_define_inputs dictionary should contain at least a key 'params'.")
        utils.check_set_dict_keys(self._model_define_inputs, ["default_NN"], [True], verbose=verbose_sub)
        utils.check_set_dict_keys(self._model_bijector_inputs, ["default_bijector"], [True], verbose=verbose_sub)
